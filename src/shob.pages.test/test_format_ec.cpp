...
namespace shob::pages::test
{
    ...
    void testFormatEc::test_2024_2025()
    {
        ...
        ASSERT_EQ(lines.data.size(), 257);
        EXPECT_EQ(lines.data[29], "<tr><td>Atalanta Bergamo +</td><td>8</td><td>15</td><td>14</td></tr>") << "check + after team name";
    }

    void testFormatEc::test_2023_2024()
    {
        const auto lines = fmt_ec.get_season(general::season(2023));
        EXPECT_EQ(lines.data.size(), 167);
    }

    void testFormatEc::test_1994_1995()
    {
        const auto lines = fmt_ec.get_season(general::season(1994));
        ASSERT_EQ(lines.data.size(), 108);
        EXPECT_EQ(lines.data[25], "<tr><td>Ajax</td><td>6</td><td>10</td><td>7</td></tr>") << "check 2 points for a win";
    }
    ...
    void testFormatEc::test_2019_2020_UK()
    {
        ...
        ASSERT_EQ(lines.data.size(), 162);
        EXPECT_EQ(lines.data[20], "<br>Sevilla wins the Europa League with two goals of Luuk de Jong.") << "check UK summary";
        EXPECT_EQ(lines.data[15], "Due to the Covid-19 pandemic, the tournament was suspended") << "check reading comma between quotes";
        EXPECT_EQ(lines.data[60], "<tr><td></td><td></td><td></td><td><b>F I N A L:</b></td></tr>") << "check language in route2final";
    }
}
